...
// FloxFlake binds a nix flake (a workload-package catalog) to a Flux source, so FloxEnvs
// can reference its outputs symbolically and the controller resolves them from the
// source's reconciled in-cluster artifact (exact commit, no external fetch/token).
